...
function parsePayment(value) {
    ...
    if (mode === "MORTGAGE")
        return { mode, inputs: { mode, downPaymentPercent: number(inputs.downPaymentPercent, "payment.inputs.downPaymentPercent"), annualRatePercent: string(inputs.annualRatePercent, "payment.inputs.annualRatePercent"), termYears: number(inputs.termYears, "payment.inputs.termYears") }, result: parseMortgage(payment.result) };
...
export function parseProposalContextResponse(value) {
    ...
    if (![1, 2, PROPOSAL_CONTEXT_SCHEMA_VERSION].includes(schemaVersion))
